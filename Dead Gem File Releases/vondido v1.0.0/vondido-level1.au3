;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;Dead Gem: NYC Level 1
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#include <GuiConstants.au3>
Global $Paused
HotKeySet("{F2}", "Start_Game")

GuiCreate("Dead Gem: NYC - Level 1", 401, 384,)

$Full_Label = GUICtrlCreateLabel("", 49, 59, 302, 302, $SS_BLACKFRAME)
$x1 = 50
$x2 = 80
$x3 = 110
$x4 = 140
$x5 = 170
$x6 = 200
$x7 = 230
$x8 = 260
$x9 = 290
$x10 = 320
$y1 = 60
$y2 = 90
$y3 = 120
$y4 = 150
$y5 = 180
$y6 = 210
$y7 = 240
$y8 = 270
$y9 = 300
$y10 = 330
$Label_1 = GuiCtrlCreateLabel("", $x1, $y1, 30, 30, $SS_BLACKRECT)
$Label_2 = GuiCtrlCreateLabel("", $x2, $y1, 30, 30, $SS_BLACKRECT)
$Label_3 = GuiCtrlCreateLabel("", $x3, $y1, 30, 30)
$Label_4 = GuiCtrlCreateLabel("", $x4, $y1, 30, 30)
$Label_5 = GuiCtrlCreateLabel("", $x5, $y1, 30, 30, $SS_BLACKRECT)
$Label_6 = GuiCtrlCreateLabel("", $x6, $y1, 30, 30)
$Label_7 = GuiCtrlCreateLabel("", $x7, $y1, 30, 30, $SS_BLACKRECT)
$Label_8 = GuiCtrlCreateLabel("", $x8, $y1, 30, 30)
$Label_9 = GuiCtrlCreateLabel("", $x9, $y1, 30, 30)
$Label_10 = GuiCtrlCreateLabel("", $x10, $y1, 30, 30)
$Label_11 = GuiCtrlCreateLabel("", $x1, $y2, 30, 30)
$Label_12 = GuiCtrlCreateLabel("", $x2, $y2, 30, 30, $SS_BLACKRECT)
$Label_13 = GuiCtrlCreateLabel("", $x3, $y2, 30, 30, $SS_BLACKRECT)
$Label_14 = GuiCtrlCreateLabel("", $x4, $y2, 30, 30)
$Label_15 = GuiCtrlCreateLabel("", $x5, $y2, 30, 30, $SS_BLACKRECT)
$Label_16 = GuiCtrlCreateLabel("", $x6, $y2, 30, 30)
$Label_17 = GuiCtrlCreateLabel("", $x7, $y2, 30, 20)
$Label_18 = GuiCtrlCreateLabel("", $x8, $y2, 30, 30)
$Label_19 = GuiCtrlCreateLabel("", $x9, $y2, 30, 30, $SS_BLACKRECT)
$Label_20 = GuiCtrlCreateLabel("", $x10, $y2, 30, 30, $SS_BLACKRECT)
$Label_21 = GuiCtrlCreateLabel("", $x1, $y3, 30, 30)
$Label_22 = GuiCtrlCreateLabel("", $x2, $y3, 30, 30, $SS_BLACKRECT)
$Label_23 = GuiCtrlCreateLabel("", $x3, $y3, 30, 30)
$Label_24 = GuiCtrlCreateLabel("", $x4, $y3, 30, 30)
$Label_25 = GuiCtrlCreateLabel("", $x5, $y3, 30, 30, $SS_BLACKRECT)
$Label_26 = GuiCtrlCreateLabel("", $x6, $y3, 30, 30, $SS_BLACKRECT)
$Label_27 = GuiCtrlCreateLabel("", $x7, $y3, 30, 30, $SS_BLACKRECT)
$Label_28 = GuiCtrlCreateLabel("", $x8, $y3, 30, 30)
$Label_29 = GuiCtrlCreateLabel("", $x9, $y3, 30, 30)
$Label_30 = GuiCtrlCreateLabel("", $x10, $y3, 30, 30)
$Label_31 = GuiCtrlCreateLabel("", $x1, $y4, 30, 30)
$Label_32 = GuiCtrlCreateLabel("", $x2, $y4, 30, 30)
$Label_33 = GuiCtrlCreateLabel("", $x3, $y4, 30, 30, $SS_BLACKRECT)
$Label_34 = GuiCtrlCreateLabel("", $x4, $y4, 30, 30)
$Label_35 = GuiCtrlCreateLabel("", $x5, $y4, 30, 30)
$Label_36 = GuiCtrlCreateLabel("", $x6, $y4, 30, 30)
$Label_37 = GuiCtrlCreateLabel("", $x7, $y4, 30, 30, $SS_BLACKRECT)
$Label_38 = GuiCtrlCreateLabel("", $x8, $y4, 30, 30, $SS_BLACKRECT)
$Label_39 = GuiCtrlCreateLabel("", $x9, $y4, 30, 30)
$Label_40 = GuiCtrlCreateLabel("", $x10, $y4, 30, 30, $SS_BLACKRECT)
$Label_41 = GuiCtrlCreateLabel("", $x1, $y5, 30, 30, $SS_BLACKRECT)
$Label_42 = GuiCtrlCreateLabel("", $x2, $y5, 30, 30)
$Label_43 = GuiCtrlCreateLabel("", $x3, $y5, 30, 30, $SS_BLACKRECT)
$Label_44 = GuiCtrlCreateLabel("", $x4, $y5, 30, 30)
$Label_45 = GuiCtrlCreateLabel("", $x5, $y5, 30, 30, $SS_BLACKRECT)
$Label_46 = GuiCtrlCreateLabel("", $x6, $y5, 30, 30)
$Label_47 = GuiCtrlCreateLabel("", $x7, $y5, 30, 30)
$Label_48 = GuiCtrlCreateLabel("", $x8, $y5, 30, 30)
$Label_49 = GuiCtrlCreateLabel("", $x9, $y5, 30, 30)
$Label_50 = GuiCtrlCreateLabel("", $x10, $y5, 30, 30, $SS_BLACKRECT)
$Label_51 = GuiCtrlCreateLabel("", $x1, $y6, 30, 30)
$Label_52 = GuiCtrlCreateLabel("", $x2, $y6, 30, 30)
$Label_53 = GuiCtrlCreateLabel("", $x3, $y6, 30, 30)
$Label_54 = GuiCtrlCreateLabel("", $x4, $y6, 30, 30)
$Label_55 = GuiCtrlCreateLabel("", $x5, $y6, 30, 30, $SS_BLACKRECT)
$Label_56 = GuiCtrlCreateLabel("", $x6, $y6, 30, 30)
$Label_57 = GuiCtrlCreateLabel("", $x7, $y6, 30, 30, $SS_BLACKRECT)
$Label_58 = GuiCtrlCreateLabel("", $x8, $y6, 30, 30, $SS_BLACKRECT)
$Label_59 = GuiCtrlCreateLabel("", $x9, $y6, 30, 30)
$Label_60 = GuiCtrlCreateLabel("", $x10, $y6, 30, 30)
$Label_61 = GuiCtrlCreateLabel("", $x1, $y7, 30, 30, $SS_BLACKRECT)
$Label_62 = GuiCtrlCreateLabel("", $x2, $y7, 30, 30, $SS_BLACKRECT)
$Label_63 = GuiCtrlCreateLabel("", $x3, $y7, 30, 30, $SS_BLACKRECT)
$Label_64 = GuiCtrlCreateLabel("", $x4, $y7, 30, 30)
$Label_65 = GuiCtrlCreateLabel("", $x5, $y7, 30, 30, $SS_BLACKRECT)
$Label_66 = GuiCtrlCreateLabel("", $x6, $y7, 30, 30)
$Label_67 = GuiCtrlCreateLabel("", $x7, $y7, 30, 30)
$Label_68 = GuiCtrlCreateLabel("", $x8, $y7, 30, 30)
$Label_69 = GuiCtrlCreateLabel("", $x9, $y7, 30, 30, $SS_BLACKRECT)
$Label_70 = GuiCtrlCreateLabel("", $x10, $y7, 30, 30)
$Label_71 = GuiCtrlCreateLabel("", $x1, $y8, 30, 30)
$Label_72 = GuiCtrlCreateLabel("", $x2, $y8, 30, 30)
$Label_73 = GuiCtrlCreateLabel("", $x3, $y8, 30, 30)
$Label_74 = GuiCtrlCreateLabel("", $x4, $y8, 30, 30)
$Label_75 = GuiCtrlCreateLabel("", $x5, $y8, 30, 30, $SS_BLACKRECT)
$Label_76 = GuiCtrlCreateLabel("", $x6, $y8, 30, 30)
$Label_77 = GuiCtrlCreateLabel("", $x7, $y8, 30, 30, $SS_BLACKRECT)
$Label_78 = GuiCtrlCreateLabel("", $x8, $y8, 30, 30)
$Label_79 = GuiCtrlCreateLabel("", $x9, $y8, 30, 30, $SS_BLACKRECT)
$Label_80 = GuiCtrlCreateLabel("", $x10, $y8, 30, 30)
$Label_81 = GuiCtrlCreateLabel("", $x1, $y9, 30, 30)
$Label_82 = GuiCtrlCreateLabel("", $x2, $y9, 30, 30, $SS_BLACKRECT)
$Label_83 = GuiCtrlCreateLabel("", $x3, $y9, 30, 30, $SS_BLACKRECT)
$Label_84 = GuiCtrlCreateLabel("", $x4, $y9, 30, 30, $SS_BLACKRECT)
$Label_85 = GuiCtrlCreateLabel("", $x5, $y9, 30, 30, $SS_BLACKRECT)
$Label_86 = GuiCtrlCreateLabel("", $x6, $y9, 30, 30, $SS_BLACKRECT)
$Label_87 = GuiCtrlCreateLabel("", $x7, $y9, 30, 30)
$Label_88 = GuiCtrlCreateLabel("", $x8, $y9, 30, 30)
$Label_89 = GuiCtrlCreateLabel("", $x9, $y9, 30, 30, $SS_BLACKRECT)
$Label_90 = GuiCtrlCreateLabel("", $x10, $y9, 30, 30)
$Label_91 = GuiCtrlCreateLabel("", $x1, $y10, 30, 30)
$Label_92 = GuiCtrlCreateLabel("", $x2, $y10, 30, 30)
$Label_93 = GuiCtrlCreateLabel("", $x3, $y10, 30, 30)
$Label_94 = GuiCtrlCreateLabel("", $x4, $y10, 30, 30, $SS_BLACKRECT)
$Label_95 = GuiCtrlCreateLabel("", $x5, $y10, 30, 30)
$Label_96 = GuiCtrlCreateLabel("", $x6, $y10, 30, 30)
$Label_97 = GuiCtrlCreateLabel("", $x7, $y10, 30, 30)
$Label_98 = GuiCtrlCreateLabel("", $x8, $y10, 30, 30, $SS_BLACKRECT)
$Label_99 = GuiCtrlCreateLabel("", $x9, $y10, 30, 30, $SS_BLACKRECT)
$Label_100 = GuiCtrlCreateLabel("", $x10, $y10, 30, 30)
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;Pics
$Person = GUICtrlCreatePic(@ScriptDir & "\Person.bmp", $x1, $y2, 30, 30, -1, $WS_EX_TOPMOST)
$Snake1 = GUICtrlCreatePic(@ScriptDir & "\Snake.bmp", $x4, $y1, 30, 30)
$Snake2 = GUICtrlCreatePic(@ScriptDir & "\Snake.bmp", $x9, $y1, 30, 30)
$Gem1 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x3, $y1, 30, 30)
$Gem2 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x6, $y1, 30, 30)
$Gem3 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x10, $y3, 30, 30)
$Gem4 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x1, $y6, 30, 30)
$Gem5 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x6, $y8, 30, 30)
$Gem6 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x3, $y10, 30, 30)
$Gem7 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x5, $y10, 30, 30)
$Gem8 = GUICtrlCreatePic(@ScriptDir & "\Gem.bmp", $x10, $y1, 30, 30)
$Key = GUICtrlCreatePic(@ScriptDir & "\Key.bmp", $x3, $y3, 30, 30)
$Keyhole = GUICtrlCreatePic(@ScriptDir & "\Keyhole.bmp", $x10, $y10, 30, 30)
;End of Pics
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
$Lives_Label = GuiCtrlCreateLabel("Lives:", 30, 10, 30, 20)
$Seperator = GuiCtrlCreateLabel("------------------------------------------------------------------------------------------------------------------------------------------------", 0, 40, 400, 10)
$Time_Label = GuiCtrlCreateLabel("Time:", 90, 10, 30, 20)
$Gem_Count = GuiCtrlCreateLabel("Gems Collected:", 190, 10, 80, 20)
$Lives_Left = GuiCtrlCreateLabel("3", 60, 10, 20, 20)
$Time_Counter = GuiCtrlCreateLabel("300", 141, 10, 30, 20)
$Gems = GuiCtrlCreateLabel("0", 280, 10, 40, 20)
$Person_Pos = 11
$Snake1_Pos = 4
$Snake2_Pos = 9
$Have_Key = 0
$Collected = 0
$SnakeMove1 = 0
$SnakeMove2 = 0
$Lives = 3
$Gem_1 = 0
$Gem_2 = 0
$Gem_3 = 0
$Gem_4 = 0
$Gem_5 = 0
$Gem_6 = 0
$Gem_7 = 0
$Gem_8 = 0
	
GuiSetState()

While Not $Paused
	$msg = GuiGetMsg()
	If $msg = $GUI_EVENT_CLOSE Then
		Exit
	EndIf
WEnd

While 1
	GUICtrlSetData(123, $Collected)
	$Rounded = Round(TimerDiff($Time)/1000) - 300
	If $Rounded = 0 Then
		MsgBox(0, "Uh Oh!", "Time Up")
	EndIf
	GUICtrlSetData(122, $Rounded * -1)
	If $Person_Pos = $Snake1_Pos Or $Person_Pos = $Snake2_Pos Then
		If $Lives = 0 Then
			MsgBox(0, "Uh Oh!", "Game Over")
		Else
			Restart()
			GUICtrlSetData(121, $Lives - 1)
			$Lives = $Lives - 1
		EndIf
	EndIf
	$msg = GuiGetMsg()
	If $msg = $GUI_EVENT_CLOSE Then
		Exit
	EndIf
WEnd

Func Start_Game()
	Global $Time = TimerInit()
	$Paused = Not $Paused
	$Lives = 2
	GUICtrlSetData(121, 2)
	HotKeySet("{Up}", "Move_Up")
	HotKeySet("{Down}", "Move_Down")
	HotKeySet("{Left}", "Move_Left")
	HotKeySet("{Right}", "Move_Right")
	AdlibEnable("MoveSnakes", 700)
EndFunc

Func Move_Up()
	Select
		Case $Person_Pos = 14
			GUICtrlSetPos(104, $x4, $y1, 30, 30)
			$Person_Pos = 4
		Case $Person_Pos = 16
			GUICtrlSetPos(104, $x6, $y1, 30, 30)
			$Person_Pos = 6
			If $Gem_1 = 0 Then
				$Collected = $Collected + 1
				$Gem_1 = 1
				GUICtrlDelete(108)
			EndIf
		Case $Person_Pos = 18
			GUICtrlSetPos(104, $x8, $y1, 30, 30)
			$Person_Pos = 8
		Case $Person_Pos = 21
			GUICtrlSetPos(104, $x1, $y2, 30, 30)
			$Person_Pos = 11
		Case $Person_Pos = 24
			GUICtrlSetPos(104, $x4, $y2, 30, 30)
			$Person_Pos = 14
		Case $Person_Pos = 28
			GUICtrlSetPos(104, $x8, $y2, 30, 30)
			$Person_Pos = 18
		Case $Person_Pos = 31
			GUICtrlSetPos(104, $x1, $y3, 30, 30)
			$Person_Pos = 21
		Case $Person_Pos = 34
			GUICtrlSetPos(104, $x4, $y3, 30, 30)
			$Person_Pos = 24
		Case $Person_Pos = 39
			GUICtrlSetPos(104, $x9, $y3, 30, 30)
			$Person_Pos = 29
		Case $Person_Pos = 42
			GUICtrlSetPos(104, $x2, $y4, 30, 30)
			$Person_Pos = 32
		Case $Person_Pos = 44
			GUICtrlSetPos(104, $x4, $y4, 30, 30)
			$Person_Pos = 34
		Case $Person_Pos = 46
			GUICtrlSetPos(104, $x6, $y4, 30, 30)
			$Person_Pos = 36
		Case $Person_Pos = 49
			GUICtrlSetPos(104, $x9, $y4, 30, 30)
			$Person_Pos = 39
		Case $Person_Pos = 52
			GUICtrlSetPos(104, $x2, $y5, 30, 30)
			$Person_Pos = 42
		Case $Person_Pos = 54
			GUICtrlSetPos(104, $x4, $y5, 30, 30)
			$Person_Pos = 44
		Case $Person_Pos = 56
			GUICtrlSetPos(104, $x6, $y5, 30, 30)
			$Person_Pos = 46
		Case $Person_Pos = 59
			GUICtrlSetPos(104, $x9, $y5, 30, 30)
			$Person_Pos = 49
		Case $Person_Pos = 64
			GUICtrlSetPos(104, $x4, $y6, 30, 30)
			$Person_Pos = 54
		Case $Person_Pos = 66
			GUICtrlSetPos(104, $x6, $y6, 30, 30)
			$Person_Pos = 56
		Case $Person_Pos = 70
			GUICtrlSetPos(104, $x10, $y6, 30, 30)
			$Person_Pos = 60
		Case $Person_Pos = 74
			GUICtrlSetPos(104, $x4, $y7, 30, 30)
			$Person_Pos = 64
		Case $Person_Pos = 76
			GUICtrlSetPos(104, $x6, $y7, 30, 30)
			$Person_Pos = 66
		Case $Person_Pos = 78
			GUICtrlSetPos(104, $x8, $y7, 30, 30)
			$Person_Pos = 68
		Case $Person_Pos = 80
			GUICtrlSetPos(104, $x10, $y7, 30, 30)
			$Person_Pos = 70
		Case $Person_Pos = 81
			GUICtrlSetPos(104, $x1, $y8, 30, 30)
			$Person_Pos = 71
		Case $Person_Pos = 88
			GUICtrlSetPos(104, $x8, $y8, 30, 30)
			$Person_Pos = 78
		Case $Person_Pos = 90
			GUICtrlSetPos(104, $x10, $y8, 30, 30)
			$Person_Pos = 80
		Case $Person_Pos = 91
			GUICtrlSetPos(104, $x1, $y9, 30, 30)
			$Person_Pos = 81
		Case $Person_Pos = 97
			GUICtrlSetPos(104, $x7, $y9, 30, 30)
			$Person_Pos = 87
	EndSelect
EndFunc

Func Move_Down()
	Select
		Case $Person_Pos = 4
			GUICtrlSetPos(104, $x4, $y2, 30, 30)
			$Person_Pos = 14
		Case $Person_Pos = 6
			GUICtrlSetPos(104, $x6, $y2, 30, 30)
			$Person_Pos = 16
		Case $Person_Pos = 8
			GUICtrlSetPos(104, $x8, $y2, 30, 30)
			$Person_Pos = 18
		Case $Person_Pos = 11
			GUICtrlSetPos(104, $x1, $y3, 30, 30)
			$Person_Pos = 21
		Case $Person_Pos = 14
			GUICtrlSetPos(104, $x4, $y3, 30, 30)
			$Person_Pos = 24
		Case $Person_Pos = 18
			GUICtrlSetPos(104, $x8, $y3, 30, 30)
			$Person_Pos = 28
		Case $Person_Pos = 21
			GUICtrlSetPos(104, $x1, $y4, 30, 30)
			$Person_Pos = 31
		Case $Person_Pos = 24
			GUICtrlSetPos(104, $x4, $y4, 30, 30)
			$Person_Pos = 34
		Case $Person_Pos = 29
			GUICtrlSetPos(104, $x9, $y4, 30, 30)
			$Person_Pos = 39
		Case $Person_Pos = 32
			GUICtrlSetPos(104, $x2, $y5, 30, 30)
			$Person_Pos = 42
		Case $Person_Pos = 34
			GUICtrlSetPos(104, $x4, $y5, 30, 30)
			$Person_Pos = 44
		Case $Person_Pos = 36
			GUICtrlSetPos(104, $x6, $y5, 30, 30)
			$Person_Pos = 46
		Case $Person_Pos = 39
			GUICtrlSetPos(104, $x9, $y5, 30, 30)
			$Person_Pos = 49
		Case $Person_Pos = 42
			GUICtrlSetPos(104, $x2, $y6, 30, 30)
			$Person_Pos = 52
		Case $Person_Pos = 44
			GUICtrlSetPos(104, $x4, $y6, 30, 30)
			$Person_Pos = 54
		Case $Person_Pos = 46
			GUICtrlSetPos(104, $x6, $y6, 30, 30)
			$Person_Pos = 56
		Case $Person_Pos = 49
			GUICtrlSetPos(104, $x9, $y6, 30, 30)
			$Person_Pos = 59
		Case $Person_Pos = 54
			GUICtrlSetPos(104, $x4, $y7, 30, 30)
			$Person_Pos = 64
		Case $Person_Pos = 56
			GUICtrlSetPos(104, $x6, $y7, 30, 30)
			$Person_Pos = 66
		Case $Person_Pos = 60
			GUICtrlSetPos(104, $x10, $y7, 30, 30)
			$Person_Pos = 70
		Case $Person_Pos = 64
			GUICtrlSetPos(104, $x4, $y8, 30, 30)
			$Person_Pos = 74
		Case $Person_Pos = 66
			GUICtrlSetPos(104, $x6, $y8, 30, 30)
			$Person_Pos = 76
			If $Gem_2 = 0 Then
				$Collected = $Collected + 1
				$Gem_2 = 1
				GUICtrlDelete(111)
			EndIf
		Case $Person_Pos = 68
			GUICtrlSetPos(104, $x8, $y8, 30, 30)
			$Person_Pos = 78
		Case $Person_Pos = 70
			GUICtrlSetPos(104, $x10, $y8, 30, 30)
			$Person_Pos = 80
		Case $Person_Pos = 71
			GUICtrlSetPos(104, $x1, $y9, 30, 30)
			$Person_Pos = 81
		Case $Person_Pos = 78
			GUICtrlSetPos(104, $x8, $y9, 30, 30)
			$Person_Pos = 88
		Case $Person_Pos = 80
			GUICtrlSetPos(104, $x10, $y9, 30, 30)
			$Person_Pos = 90
		Case $Person_Pos = 81
			GUICtrlSetPos(104, $x1, $y10, 30, 30)
			$Person_Pos = 91
		Case $Person_Pos = 87
			GUICtrlSetPos(104, $x7, $y10, 30, 30)
			$Person_Pos = 97
		Case $Person_Pos = 90
			If $Have_Key = 1 And $Collected = 8 Then
				MsgBox(0, "Congradulations!", "Level 1 complete, click ""OK"" to continue on to Level 2.")
				GUIDelete()
				Exit
			Else
				$Person_Pos = 90
			EndIf
	EndSelect
EndFunc

Func Move_Left()
	Select
		Case $Person_Pos = 4
			GUICtrlSetPos(104, $x3, $y1, 30, 30)
			$Person_Pos = 3
			If $Gem_3 = 0 Then
				$Collected = $Collected + 1
				$Gem_3 = 1
				GUICtrlDelete(107)
			EndIf
		Case $Person_Pos = 9
			GUICtrlSetPos(104, $x8, $y1, 30, 30)
			$Person_Pos = 8
		Case $Person_Pos = 10
			GUICtrlSetPos(104, $x9, $y1, 30, 30)
			$Person_Pos = 9
		Case $Person_Pos = 17
			GUICtrlSetPos(104, $x6, $y2, 30, 30)
			$Person_Pos = 16
		Case $Person_Pos = 18
			GUICtrlSetPos(104, $x7, $y2, 30, 30)
			$Person_Pos = 17
		Case $Person_Pos = 24
			GUICtrlSetPos(104, $x3, $y3, 30, 30)
			$Person_Pos = 23
			If $Have_Key = 0 Then
				$Have_Key = 1
				GUICtrlSetPos(115, 340, 10, 30, 30)
			EndIf
		Case $Person_Pos = 29
			GUICtrlSetPos(104, $x8, $y3, 30, 30)
			$Person_Pos = 28
		Case $Person_Pos = 30
			GUICtrlSetPos(104, $x9, $y3, 30, 30)
			$Person_Pos = 29
		Case $Person_Pos = 32
			GUICtrlSetPos(104, $x1, $y4, 30, 30)
			$Person_Pos = 31
		Case $Person_Pos = 35
			GUICtrlSetPos(104, $x4, $y4, 30, 30)
			$Person_Pos = 34
		Case $Person_Pos = 36
			GUICtrlSetPos(104, $x5, $y4, 30, 30)
			$Person_Pos = 35
		Case $Person_Pos = 47
			GUICtrlSetPos(104, $x6, $y5, 30, 30)
			$Person_Pos = 46
		Case $Person_Pos = 48
			GUICtrlSetPos(104, $x7, $y5, 30, 30)
			$Person_Pos = 47
		Case $Person_Pos = 49
			GUICtrlSetPos(104, $x8, $y5, 30, 30)
			$Person_Pos = 48
		Case $Person_Pos = 52
			GUICtrlSetPos(104, $x1, $y6, 30, 30)
			$Person_Pos = 51
			If $Gem_4 = 0 Then
				$Collected = $Collected + 1
				$Gem_4 = 1
				GUICtrlDelete(110)
			EndIf
		Case $Person_Pos = 53
			GUICtrlSetPos(104, $x2, $y6, 30, 30)
			$Person_Pos = 52
		Case $Person_Pos = 54
			GUICtrlSetPos(104, $x3, $y6, 30, 30)
			$Person_Pos = 53
		Case $Person_Pos = 60
			GUICtrlSetPos(104, $x9, $y6, 30, 30)
			$Person_Pos = 59
		Case $Person_Pos = 67
			GUICtrlSetPos(104, $x6, $y7, 30, 30)
			$Person_Pos = 66
		Case $Person_Pos = 68
			GUICtrlSetPos(104, $x7, $y7, 30, 30)
			$Person_Pos = 67
		Case $Person_Pos = 72
			GUICtrlSetPos(104, $x1, $y8, 30, 30)
			$Person_Pos = 71
		Case $Person_Pos = 73
			GUICtrlSetPos(104, $x2, $y8, 30, 30)
			$Person_Pos = 72
		Case $Person_Pos = 74
			GUICtrlSetPos(104, $x3, $y8, 30, 30)
			$Person_Pos = 73
		Case $Person_Pos = 88
			GUICtrlSetPos(104, $x7, $y9, 30, 30)
			$Person_Pos = 87
		Case $Person_Pos = 92
			GUICtrlSetPos(104, $x1, $y10, 30, 30)
			$Person_Pos = 91
		Case $Person_Pos = 93
			GUICtrlSetPos(104, $x2, $y10, 30, 30)
			$Person_Pos = 92
		Case $Person_Pos = 96
			GUICtrlSetPos(104, $x5, $y10, 30, 30)
			$Person_Pos = 95
			If $Gem_5 = 0 Then
				$Collected = $Collected + 1
				$Gem_5 = 1
				GUICtrlDelete(113)
			EndIf
		Case $Person_Pos = 97
			GUICtrlSetPos(104, $x6, $y10, 30, 30)
			$Person_Pos = 96
	EndSelect
EndFunc

Func Move_Right()
	Select
		Case $Person_Pos = 3
			GUICtrlSetPos(104, $x4, $y1, 30, 30)
			$Person_Pos = 4
		Case $Person_Pos = 8
			GUICtrlSetPos(104, $x9, $y1, 30, 30)
			$Person_Pos = 9
		Case $Person_Pos = 9
			GUICtrlSetPos(104, $x10, $y1, 30, 30)
			$Person_Pos = 10
			If $Gem_6 = 0 Then
				$Collected = $Collected + 1
				$Gem_6 = 1
				GUICtrlDelete(114)
			EndIf
		Case $Person_Pos = 16
			GUICtrlSetPos(104, $x7, $y2, 30, 30)
			$Person_Pos = 17
		Case $Person_Pos = 17
			GUICtrlSetPos(104, $x8, $y2, 30, 30)
			$Person_Pos = 18
		Case $Person_Pos = 23
			GUICtrlSetPos(104, $x4, $y3, 30, 30)
			$Person_Pos = 24
		Case $Person_Pos = 28
			GUICtrlSetPos(104, $x9, $y3, 30, 30)
			$Person_Pos = 29
		Case $Person_Pos = 29
			GUICtrlSetPos(104, $x10, $y3, 30, 30)
			$Person_Pos = 30
			If $Gem_7 = 0 Then
				$Collected = $Collected + 1
				$Gem_7 = 1
				GUICtrlDelete(109)
			EndIf
		Case $Person_Pos = 31
			GUICtrlSetPos(104, $x2, $y4, 30, 30)
			$Person_Pos = 32
		Case $Person_Pos = 34
			GUICtrlSetPos(104, $x5, $y4, 30, 30)
			$Person_Pos = 35
		Case $Person_Pos = 35
			GUICtrlSetPos(104, $x6, $y4, 30, 30)
			$Person_Pos = 36
		Case $Person_Pos = 46
			GUICtrlSetPos(104, $x7, $y5, 30, 30)
			$Person_Pos = 47
		Case $Person_Pos = 47
			GUICtrlSetPos(104, $x8, $y5, 30, 30)
			$Person_Pos = 48
		Case $Person_Pos = 48
			GUICtrlSetPos(104, $x9, $y5, 30, 30)
			$Person_Pos = 49
		Case $Person_Pos = 51
			GUICtrlSetPos(104, $x2, $y6, 30, 30)
			$Person_Pos = 52
		Case $Person_Pos = 52
			GUICtrlSetPos(104, $x3, $y6, 30, 30)
			$Person_Pos = 53
		Case $Person_Pos = 53
			GUICtrlSetPos(104, $x4, $y6, 30, 30)
			$Person_Pos = 54
		Case $Person_Pos = 59
			GUICtrlSetPos(104, $x10, $y6, 30, 30)
			$Person_Pos = 60
		Case $Person_Pos = 66
			GUICtrlSetPos(104, $x7, $y7, 30, 30)
			$Person_Pos = 67
		Case $Person_Pos = 67
			GUICtrlSetPos(104, $x8, $y7, 30, 30)
			$Person_Pos = 68
		Case $Person_Pos = 71
			GUICtrlSetPos(104, $x2, $y8, 30, 30)
			$Person_Pos = 72
		Case $Person_Pos = 72
			GUICtrlSetPos(104, $x3, $y8, 30, 30)
			$Person_Pos = 73
		Case $Person_Pos = 73
			GUICtrlSetPos(104, $x4, $y8, 30, 30)
			$Person_Pos = 74
		Case $Person_Pos = 87
			GUICtrlSetPos(104, $x8, $y9, 30, 30)
			$Person_Pos = 88
		Case $Person_Pos = 91
			GUICtrlSetPos(104, $x2, $y10, 30, 30)
			$Person_Pos = 92
		Case $Person_Pos = 92
			GUICtrlSetPos(104, $x3, $y10, 30, 30)
			$Person_Pos = 93
			If $Gem_8 = 0 Then
				$Collected = $Collected + 1
				$Gem_8 = 1
				GUICtrlDelete(112)
			EndIf
		Case $Person_Pos = 95
			GUICtrlSetPos(104, $x6, $y10, 30, 30)
			$Person_Pos = 96
		Case $Person_Pos = 96
			GUICtrlSetPos(104, $x7, $y10, 30, 30)
			$Person_Pos = 97
	EndSelect
EndFunc

Func MoveSnakes()
	If $SnakeMove1 = 0 Then
		Select
			Case $Snake1_Pos = 4
				GUICtrlSetPos(105, $x4, $y2, 30, 30)
				$Snake1_Pos = 14
			Case $Snake1_Pos = 14
				GUICtrlSetPos(105, $x4, $y3, 30, 30)
				$Snake1_Pos = 24
			Case $Snake1_Pos = 24
				GUICtrlSetPos(105, $x4, $y4, 30, 30)
				$Snake1_Pos = 34
			Case $Snake1_Pos = 34
				GUICtrlSetPos(105, $x4, $y5, 30, 30)
				$Snake1_Pos = 44
			Case $Snake1_Pos = 44
				GUICtrlSetPos(105, $x4, $y6, 30, 30)
				$Snake1_Pos = 54
			Case $Snake1_Pos = 54
				GUICtrlSetPos(105, $x4, $y7, 30, 30)
				$Snake1_Pos = 64
			Case $Snake1_Pos = 64
				GUICtrlSetPos(105, $x4, $y8, 30, 30)
				$Snake1_Pos = 74
			Case $Snake1_Pos = 74
				GUICtrlSetPos(105, $x3, $y8, 30, 30)
				$Snake1_Pos = 73
			Case $Snake1_Pos = 73
				GUICtrlSetPos(105, $x2, $y8, 30, 30)
				$Snake1_Pos = 72
			Case $Snake1_Pos = 72
				GUICtrlSetPos(105, $x1, $y8, 30, 30)
				$Snake1_Pos = 71
			Case $Snake1_Pos = 71
				GUICtrlSetPos(105, $x1, $y9, 30, 30)
				$Snake1_Pos = 81
			Case $Snake1_Pos = 81
				GUICtrlSetPos(105, $x1, $y10, 30, 30)
				$Snake1_Pos = 91
			Case $Snake1_Pos = 91
				GUICtrlSetPos(105, $x2, $y10, 30, 30)
				$Snake1_Pos = 92
			Case $Snake1_Pos = 92
				$SnakeMove1 = 1
		EndSelect
	EndIf
	If $SnakeMove1 = 1 Then
		Select
			Case $Snake1_Pos = 92
				GUICtrlSetPos(105, $x1, $y10, 30, 30)
				$Snake1_Pos = 91
			Case $Snake1_Pos = 91
				GUICtrlSetPos(105, $x1, $y9, 30, 30)
				$Snake1_Pos = 81
			Case $Snake1_Pos = 81
				GUICtrlSetPos(105, $x1, $y8, 30, 30)
				$Snake1_Pos = 71
			Case $Snake1_Pos = 71
				GUICtrlSetPos(105, $x2, $y8, 30, 30)
				$Snake1_Pos = 72
			Case $Snake1_Pos = 72
				GUICtrlSetPos(105, $x3, $y8, 30, 30)
				$Snake1_Pos = 73
			Case $Snake1_Pos = 73
				GUICtrlSetPos(105, $x4, $y8, 30, 30)
				$Snake1_Pos = 74
			Case $Snake1_Pos = 74
				GUICtrlSetPos(105, $x4, $y7, 30, 30)
				$Snake1_Pos = 64
			Case $Snake1_Pos = 64
				GUICtrlSetPos(105, $x4, $y6, 30, 30)
				$Snake1_Pos = 54
			Case $Snake1_Pos = 54
				GUICtrlSetPos(105, $x4, $y5, 30, 30)
				$Snake1_Pos = 44
			Case $Snake1_Pos = 44
				GUICtrlSetPos(105, $x4, $y4, 30, 30)
				$Snake1_Pos = 34
			Case $Snake1_Pos = 34
				GUICtrlSetPos(105, $x4, $y3, 30, 30)
				$Snake1_Pos = 24
			Case $Snake1_Pos = 24
				GUICtrlSetPos(105, $x4, $y2, 30, 30)
				$Snake1_Pos = 14
			Case $Snake1_Pos = 14
				GUICtrlSetPos(105, $x4, $y1, 30, 30)
				$Snake1_Pos = 4
				$SnakeMove1 = 0
		EndSelect
	EndIf
	If $SnakeMove2 = 0 Then
		Select
			Case $Snake2_Pos = 9
				GUICtrlSetPos(106, $x8, $y1, 30, 30)
				$Snake2_Pos = 8
			Case $Snake2_Pos = 8
				GUICtrlSetPos(106, $x8, $y2, 30, 30)
				$Snake2_Pos = 18
			Case $Snake2_Pos = 18
				GUICtrlSetPos(106, $x8, $y3, 30, 30)
				$Snake2_Pos = 28
			Case $Snake2_Pos = 28
				GUICtrlSetPos(106, $x9, $y3, 30, 30)
				$Snake2_Pos = 29
			Case $Snake2_Pos = 29
				GUICtrlSetPos(106, $x9, $y4, 30, 30)
				$Snake2_Pos = 39
			Case $Snake2_Pos = 39
				GUICtrlSetPos(106, $x9, $y5, 30, 30)
				$Snake2_Pos = 49
			Case $Snake2_Pos = 49
				GUICtrlSetPos(106, $x8, $y5, 30, 30)
				$Snake2_Pos = 48
			Case $Snake2_Pos = 48
				GUICtrlSetPos(106, $x7, $y5, 30, 30)
				$Snake2_Pos = 47
			Case $Snake2_Pos = 47
				GUICtrlSetPos(106, $x6, $y5, 30, 30)
				$Snake2_Pos = 46
			Case $Snake2_Pos = 46
				GUICtrlSetPos(106, $x6, $y6, 30, 30)
				$Snake2_Pos = 56
			Case $Snake2_Pos = 56
				GUICtrlSetPos(106, $x6, $y7, 30, 30)
				$Snake2_Pos = 66
			Case $Snake2_Pos = 66
				GUICtrlSetPos(106, $x7, $y7, 30, 30)
				$Snake2_Pos = 67
			Case $Snake2_Pos = 67
				GUICtrlSetPos(106, $x8, $y7, 30, 30)
				$Snake2_Pos = 68
			Case $Snake2_Pos = 68
				GUICtrlSetPos(106, $x8, $y8, 30, 30)
				$Snake2_Pos = 78
			Case $Snake2_Pos = 78
				GUICtrlSetPos(106, $x8, $y9, 30, 30)
				$Snake2_Pos = 88
			Case $Snake2_Pos = 88
				GUICtrlSetPos(106, $x7, $y9, 30, 30)
				$Snake2_Pos = 87
			Case $Snake2_Pos = 87
				GUICtrlSetPos(106, $x7, $y10, 30, 30)
				$Snake2_Pos = 97
			Case $Snake2_Pos = 97
				GUICtrlSetPos(106, $x6, $y10, 30, 30)
				$Snake2_Pos = 96
			Case $Snake2_Pos = 96
				$SnakeMove2 = 1
		EndSelect
	EndIf
		If $SnakeMove2 = 1 Then
		Select
			Case $Snake2_Pos = 96
				GUICtrlSetPos(106, $x7, $y10, 30, 30)
				$Snake2_Pos = 97
			Case $Snake2_Pos = 97
				GUICtrlSetPos(106, $x7, $y9, 30, 30)
				$Snake2_Pos = 87
			Case $Snake2_Pos = 87
				GUICtrlSetPos(106, $x8, $y9, 30, 30)
				$Snake2_Pos = 88
			Case $Snake2_Pos = 88
				GUICtrlSetPos(106, $x8, $y8, 30, 30)
				$Snake2_Pos = 78
			Case $Snake2_Pos = 78
				GUICtrlSetPos(106, $x8, $y7, 30, 30)
				$Snake2_Pos = 68
			Case $Snake2_Pos = 68
				GUICtrlSetPos(106, $x7, $y7, 30, 30)
				$Snake2_Pos = 67
			Case $Snake2_Pos = 67
				GUICtrlSetPos(106, $x6, $y7, 30, 30)
				$Snake2_Pos = 66
			Case $Snake2_Pos = 66
				GUICtrlSetPos(106, $x6, $y6, 30, 30)
				$Snake2_Pos = 56
			Case $Snake2_Pos = 56
				GUICtrlSetPos(106, $x6, $y5, 30, 30)
				$Snake2_Pos = 46
			Case $Snake2_Pos = 46
				GUICtrlSetPos(106, $x7, $y5, 30, 30)
				$Snake2_Pos = 47
			Case $Snake2_Pos = 47
				GUICtrlSetPos(106, $x8, $y5, 30, 30)
				$Snake2_Pos = 48
			Case $Snake2_Pos = 48
				GUICtrlSetPos(106, $x9, $y5, 30, 30)
				$Snake2_Pos = 49
			Case $Snake2_Pos = 49
				GUICtrlSetPos(106, $x9, $y4, 30, 30)
				$Snake2_Pos = 39
			Case $Snake2_Pos = 39
				GUICtrlSetPos(106, $x9, $y3, 30, 30)
				$Snake2_Pos = 29
			Case $Snake2_Pos = 29
				GUICtrlSetPos(106, $x8, $y3, 30, 30)
				$Snake2_Pos = 28
			Case $Snake2_Pos = 28
				GUICtrlSetPos(106, $x8, $y2, 30, 30)
				$Snake2_Pos = 18
			Case $Snake2_Pos = 18
				GUICtrlSetPos(106, $x8, $y1, 30, 30)
				$Snake2_Pos = 8
			Case $Snake2_Pos = 8
				GUICtrlSetPos(106, $x9, $y1, 30, 30)
				$Snake2_Pos = 9
				$SnakeMove2 = 0
		EndSelect
	EndIf
EndFunc

Func Restart()
	Global $Time = TimerInit()
	GUICtrlSetPos(104, $x1, $y2, 30, 30)
	GUICtrlSetPos(105, $x4, $y1, 30, 30)
	GUICtrlSetPos(106, $x9, $y1, 30, 30)
	GUICtrlSetPos(107, $x3, $y1, 30, 30)
	GUICtrlSetPos(108, $x6, $y1, 30, 30)
	GUICtrlSetPos(109, $x10, $y3, 30, 30)
	GUICtrlSetPos(110, $x1, $y6, 30, 30)
	GUICtrlSetPos(111, $x6, $y8, 30, 30)
	GUICtrlSetPos(112, $x3, $y10, 30, 30)
	GUICtrlSetPos(113, $x5, $y10, 30, 30)
	GUICtrlSetPos(114, $x10, $y1, 30, 30)
	GUICtrlSetPos(115, $x3, $y3, 30, 30)
	GUICtrlSetPos(116, $x10, $y10, 30, 30)
	;End of Pics
	;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	GUICtrlSetData(122, "300")
	GUICtrlSetData(123, "0")
	GUICtrlSetPos(104, $x1, $y2, 30, 30)
	$Person_Pos = 11
	GUICtrlSetPos(105, $x4, $y1, 30, 30)
	$Snake1_Pos = 4
	GUICtrlSetPos(106, $x9, $y1, 30, 30)
	$Snake2_Pos = 9
	$Have_Key = 0
	$Collected = 0
	$SnakeMove1 = 0
	$SnakeMove2 = 0
	$Gem_1 = 0
	$Gem_2 = 0
	$Gem_3 = 0
	$Gem_4 = 0
	$Gem_5 = 0
	$Gem_6 = 0
	$Gem_7 = 0
	$Gem_8 = 0
EndFunc